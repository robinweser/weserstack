...
State itself is read-only and can only be updated by invoking [actions](/state/concepts/action).
...
> Learn what a [pure function](/state/concepts/action#pure-function) is.
...
Performing side effects, such as API calls, can be done using [effects](/state/concepts/effect).
