...
A convience helper to create a form utils from a Zod schema including a scoped [useForm](/forms/api/useForm) hook, a form context provider and a controlled field component.<br />
...
import { createForm } from '@weser/forms'
...
A pre-configured [useForm](/forms/api/useForm) hook.

#### Parameters

It only accepts a a single [config](#config) parameter mirroring the [useForm](/forms/api/useForm#parameters)'s config parameters.

#### Returns

(`Object`) See [useForm](/forms/api/useForm#returns) for the return type.
...
Takes all [options](/forms/api/useField#options) as props.<br />
It accepts a single child function that receives the [field](/forms/api/useField#returns) object.
...
It exposes the same API as the [useForm](/forms/api/useForm) hook.
