...
By default, [useField](/forms/api/useField) (and [useFormField](/forms/api/useForm#useformfield) respectively) expects string values and defaults to an empty string if no initial value is provided.<br />
...
<Callout title="Note">
  ...
  `field.value` and the expected input for [update](/forms/api/useField#update).
</Callout>

<Callout title="Tip">
  There's also the [formatValue](/forms/api/useField#options) option to convert
  the value before it's passed to [props](/forms/api/useField#props) and
  [inputProps](/forms/api/useField#inputprops).
</Callout>
