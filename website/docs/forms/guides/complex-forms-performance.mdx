...
Sometimes, a single top-level [useForm](/forms/api/useForm) hook can be cumbersome to manage and/or scale.<br />
...
To mitigate both issues, we can use the [createForm](/forms/api/createForm) helper to create a form utils from a Zod schema including a scoped [useForm](/forms/api/useForm) hook, a form context provider and a controlled field component.<br />
...
The [FormProvider](/forms/api/createForm#formprovider) is a context provider that provides the form utils to its children.<br />
It allows us to use the [useFormContext](/forms/api/createForm#useformcontext) hook to access e.g. `isValidating` or `useFormField` from nested components.
