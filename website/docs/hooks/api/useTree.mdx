...
const initialTree = createNode<Node>({
  type: 'root',
})
...
(`[T, Object]`) A tuple where the first element is the tree and the second element is an object containing all the methods from the [@weser/tree](/tree) package with the first parameter (`rootNode`) stripped nd all the `Node` suffixes removed. All the mutating methods update the internal tree state instead of returning a new tree.
