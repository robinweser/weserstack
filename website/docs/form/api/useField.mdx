...
| [formatErrorMessage](#formaterrormessage) | `(error: ZodIssue, value: any, name?: string) => string` | `(error) => error.message` | A function to overwrite the default error message from Zod. Receives the Zod issue, the current value and the name of the field. |
...
(`T`) The current value of the field.
...
(`boolean`) Whether the field is touched.

### dirty

(`boolean`) Whether the field is dirty.

### valid

(`boolean`) Whether the field is valid.

### errorMessage

(`string`) A string containing the validation message.<br/>
Only returned if the field is invalid **and** touched.

### update

A function that takes [field input data](/form/api/types/T_FieldInput) and updates the field accordingly.<br />
