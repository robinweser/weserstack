...
It is very similar to [createVariableReference](/theming/api/createVariableReference) but uses React's [useId](https://react.dev/reference/react/useId) under the hood to generate a unique variable name.

```tsx
import { useCSSVariable } from '@weser/styling'
